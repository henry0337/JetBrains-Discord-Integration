...
import dev.azn9.plugins.discord.settings.options.impl.OptionProviderImpl
import dev.azn9.plugins.discord.utils.Plugin
import dev.azn9.plugins.discord.utils.gbc
...
class CustomThemeOption(text: String, description: String?, initialValue: String) : SimpleOption<CustomTemplate>(text, description, CustomTemplate(initialValue)) {
    ...
    private val textFieldInfoExtension = ExtendableTextComponent.Extension.create(AllIcons.General.Information, "Supports templates (click for more info)") {
        try {
            Desktop.getDesktop().browse(URI.create("https://github.com/Azn9/JetBrains-Discord-Integration/blob/develop/plugin/custom-theme.md"))
        } catch (e: Exception) {
            DiscordPlugin.LOG.warnLazy(e) { "Failed to open custom-theme.md" }
        }
    }
